...
class ChronobreakFilter : public rocksdb::CompactionFilter {
 public:
  bool Filter(int level, const rocksdb::Slice& key,
    ...
    bool drop = (timestamp % 5 != 0);

    if (drop) {
      std::cout << "[ChronobreakFilter] Drop key: " << key_str << "\n";
    }
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run(ToolHooks& hooks) {
    ...
    open_options_.compaction_filter_factory = std::make_shared<ChronobreakFilterFactory>();
    if (!SanityCheck()) {
      ErrorExit();
    }
    ...
    while (std::getline(benchmark_stream, name, ',')) {if (options.compaction_filter == nullptr)
      // Sanitize parameters
      num_ = FLAGS_num;
      reads_ = (FLAGS_reads < 0 ? FLAGS_num : FLAGS_reads);
      writes_ = (FLAGS_writes < 0 ? FLAGS_num : FLAGS_writes);
 ...
 private:
  ...
  void OpenDb(Options options, ToolHooks& hooks, const std::string& db_name,
    ...
    if (FLAGS_num_column_families > 1) {
      ...
      cf_opts.compaction_filter_factory = std::make_shared<ChronobreakFilterFactory>();
      ...
      Status s = DB::Open(options, db_name, cf_descs, &handles, &raw_db);
